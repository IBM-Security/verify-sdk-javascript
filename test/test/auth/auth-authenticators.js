...
describe("AuthenticatorContext - authenticators()", () => {
    ...
    it("without token - should throw error 'token is a required parameter'", () => {
        return authCtx.authenticators().then(response => {
            expect(response.response).to.not.exist;
        }).catch(error => {
            console.log(JSON.stringify(error));
            expect(error.message).to.equal('token is a required parameter');
        });
    });

    it("with invalid token (empty object) - should throw error 'Token parameter is not a valid token'", () => {
        return authCtx.authenticators({}).then(response => {
            expect(response.response).to.not.exist;
        }).catch(error => {
