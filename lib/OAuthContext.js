...
const {OAUTH_CONTEXT_CONFIG_SETTINGS_ERROR, OAUTH_CONTEXT_API_ERROR, TOKEN_ERROR} = AppConfig;
/**
 *
...
function OAuthContext(config) {
  ...
  if (!(config.flowType === 'Implicit' || config.flowType === 'AZN')){
    throw new VerifyError(OAUTH_CONTEXT_CONFIG_SETTINGS_ERROR, 'Check the flowType property in your configuration object is correct. Should be: "Implicit" or "AZN"');
  }
...
OAuthContext.prototype.isAuthenticated = async function(token){
  try {
    ...
    return payload.response.active;
  } catch (error) {
...
OAuthContext.prototype.logout = async function(path, token) {
  ...
  } else if (this.config.flowType === 'AZN' ){
...
OAuthContext.prototype.handleResponse = async function(options, tokenObj){
  ...
  try {
    ...
    if (error.status === 401 && utils.isNode()){
      ...
      let newToken = await this.refreshToken(token.refresh_token);
