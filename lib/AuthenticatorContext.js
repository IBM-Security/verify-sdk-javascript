...
AuthenticatorContext.prototype.initiateAuthenticator = function(dataObj, tokenObj) {
  ...
  let token = tokenObj || this.token;
  let path = `${this.config.tenantUrl}/v1.0/authenticators/initiation?qrcodeInResponse=true`;
  let options = {
    method: 'POST',
    url: path,
    data: {
 ...
 */
AuthenticatorContext.prototype.pollVerificationIter =  async function(authenticatorId, transactionId, tokenObj, delay, attempts) {
